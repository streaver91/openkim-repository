...
EAM_DynamoImplementation::EAM_DynamoImplementation(
    ...
    int* const ier)
    : numberParticleTypesIndex_(-1),  // initizlize index, pointer, and cached
      numberOfParticlesIndex_(-1),    // member variables
      numberContributingParticlesIndex_(-1),
      particleTypesIndex_(-1),
      coordinatesIndex_(-1),
...
int EAM_DynamoImplementation::Compute(KIM_API_model* const pkim)
{
  ...
  ier = SetComputeMutableValues(pkim, isComputeProcess_dEdr,
                                ...
                                particleTypes, get_neigh, boxSideLengths,
                                coordinates, energy, particleEnergy, forces);
  if (ier < KIM_STATUS_OK) return ier;

  ier = CheckParticleTypes(pkim, particleTypes);
  if (ier < KIM_STATUS_OK) return ier;
...
int EAM_DynamoImplementation::SetConstantValues(KIM_API_model* const pkim)
{
  ...
  // set numberModelTypes
  char* const kimModelParticleTypes
      = static_cast<char*>(
          pkim->get_model_partcl_typs(&numberModelTypes_, &ier));
  if (ier < KIM_STATUS_OK)
  {
    pkim->report_error(__LINE__, __FILE__, "get_model_partcl_typs", ier);
    return ier;
  }
  // we only really need numberModelTypes, so free the memory
  free(kimModelParticleTypes);
...
int EAM_DynamoImplementation::DetermineNBCTypeAndHalf(KIM_API_model* const pkim)
{
  ...
  char* NBC = pkim->get_NBC_method(&ier);
  if (ier < KIM_STATUS_OK)
  {
...
{ // allocate memory for particle number, mass, lattice constant, and lattice
  // type
  particleNumber_ = new int[numberModelTypes_];
  particleMass_ = new double[numberModelTypes_];
  latticeConstant_ = new double[numberModelTypes_];
  latticeType_ = new char*[numberModelTypes_];
  latticeType_[0] = new char[numberModelTypes_ * MAXLINE];
  for (int i = 1; i < numberModelTypes_; ++i)
  {
    ...
    int const numberParameterFiles)
{
  ...
  { // should be a set of Funcfl files
    ...
    if (numberModelTypes != numberParameterFiles)
    {
  ...
  else if (numberParameterFiles == 1)
  {
    ...
    if ((eamFileType == Funcfl) && (numberModelTypes != 1))
    {
      pkim->report_error(__LINE__, __FILE__, "Wrong number of parameter"
                         " files in EAM_Dynamo", KIM_STATUS_FAIL);
      return Error;
                                                    ...
                                                    FILE* const fptr)
{
  ...
  // parse number of particle types
  int number_of_types;
  ier = sscanf(particleNames_, "%d", &number_of_types);
  if ((cer == 0) || (ier != 1))
  {
    ier = KIM_STATUS_FAIL;
    pkim->report_error(__LINE__, __FILE__,
  ...
  // check consistency of particle types
  if (numberModelTypes_ != number_of_types)
  {
    ier = KIM_STATUS_FAIL;
  ...
  // parse the remainder of 4th line for particle type names
  char* const tmpnames = new char[strlen(particleNames_)+1];
  strcpy(tmpnames, particleNames_);
  char** const elems = new char*[numberModelTypes_];
  ...
  while (tmpstring != 0)
  {
    ...
    ++counter;
    if (counter >= numberModelTypes_)
    {
      break;
    }
  }
  // check to ensure that the atom types match with the KIM descriptor file
  for (int i = 0; i < numberModelTypes_; ++i)
  {
    int const acode = pkim->get_partcl_type_code(elems[i], &ier);
    if (ier < KIM_STATUS_OK)
    { // the atom type is not listed in kim file or other error
    ...
    if (acode != i)
    { // order of atom types not consistent with kim file list
      ier = KIM_STATUS_FAIL;
      pkim->report_error(__LINE__, __FILE__,
    ...
    KIM_API_model* const pkim)
{
  ...
  // get sorted list of particle names
  int temp;
  char* const kimModelParticleTypes
      = (char*) pkim->get_model_partcl_typs(&temp, &ier);
  if (ier < KIM_STATUS_OK)
  {
    pkim->report_error(__LINE__, __FILE__, "get_model_partcl_typs", ier);
    return ier;
  ...
  // correctly order the particle names
  char** const particleNames = new char*[numberModelTypes_];
  for (int i = 0; i < numberModelTypes_; ++i)
  {
    int const index = pkim->get_partcl_type_code(
        &kimModelParticleTypes[i * KIM_KEY_STRING_LENGTH], &ier);
    particleNames[index] = &kimModelParticleTypes[i * KIM_KEY_STRING_LENGTH];
  }

  // write particleNames_ string
  sprintf(particleNames_, "%d ", numberModelTypes_);
  for (int i = 0; i < numberModelTypes_; ++i)
  {
...
void EAM_DynamoImplementation::AllocateFreeParameterMemory()
{
  ...
  AllocateAndInitialize2DArray(embeddingData_, numberModelTypes_,
                               numberRhoPoints_);
  AllocateAndInitialize3DArray(densityData_, numberModelTypes_,
                               numberModelTypes_, numberRPoints_);
  AllocateAndInitialize3DArray(rPhiData_, numberModelTypes_, numberModelTypes_,
                               numberRPoints_);

  // allocate memory for coefficients
  AllocateAndInitialize2DArray(embeddingCoeff_, numberModelTypes_,
                               numberRhoPoints_ * NUMBER_SPLINE_COEFF);
  AllocateAndInitialize3DArray(densityCoeff_, numberModelTypes_,
                               numberModelTypes_,
                               numberRPoints_ * NUMBER_SPLINE_COEFF);
  AllocateAndInitialize3DArray(rPhiCoeff_, numberModelTypes_,
                               numberModelTypes_,
                               numberRPoints_ * NUMBER_SPLINE_COEFF);
}
                                                  ...
                                                  FILE* const fptr)
{
  ...
  { // read header line (partcle number, mass, lattice constant, lattice type)
    ...
    for (int j = 1; j < numberModelTypes_; ++j)
    {
  ...
  for (int i = 0; i < numberModelTypes_; ++i)
  {
    for (int j = i + 1; j < numberModelTypes_; ++j)
    {
      for (int k = 0; k < numberRPoints_; ++k)
      {
    ...
    FILE* const fptr)
{
  ...
  { // read header line (partcle number, mass, lattice constant, lattice type)
    ...
    // read "densityData"
    for (int j = 0; j < numberModelTypes_; ++j)
    {
    ...
    SetOfFuncflData const& funcflData)
{
  ...
  if (numberModelTypes_ > 1)
  {
    for (int i = 0; i < numberModelTypes_; ++i)
    {
      ...
      for (int j = 0; j < numberRPoints_; ++j)
      {
        ...
        for (int k = 1; k < numberModelTypes_; ++k)
        {
    ...
    for (int i = 0; i < numberModelTypes_; ++i)
    {
      for (int j = numberModelTypes_ - 1; j > i; --j)
      {
        for (int k = 0; k < numberRPoints_; ++k)
  ...
  { // if numberModelTypes_ == 1, don't reinterpolate
    ...
    EAMFileType const eamFileType) const
{
  ...
  int const numberCommentLines = (eamFileType == Funcfl) ?
      numberModelTypes_ : NUMBER_SETFL_COMMENT_LINES;
  pkim->setm_data(&ier, 11 * 4,
                  ...
                  numberModelTypes_, (void*)particleNumber_, 1,
                  "PARAM_FIXED_particleMass",
                  numberModelTypes_, (void*)particleMass_, 1,
                  "PARAM_FIXED_latticeConstant",
                  numberModelTypes_, (void*)latticeConstant_, 1,
                  "PARAM_FIXED_latticeType",
                  numberModelTypes_, (void*)latticeType_, 1,
  ...
  int shape_embed[2] = {numberModelTypes_, numberRhoPoints_};
  int shape_density[3] = {numberModelTypes_, numberModelTypes_, numberRPoints_};
  int shape_r_phi[3] = {numberModelTypes_, numberModelTypes_, numberRPoints_};
  ...
  pkim->setm_data(&ier, 3 * 4,
                  ...
                  numberModelTypes_ * numberRhoPoints_,
                  (void*) embeddingData_[0],
                  1,
                  //
                  "PARAM_FREE_densityData",
                  numberModelTypes_ * numberModelTypes_ * numberRPoints_,
...
//******************************************************************************
int EAM_DynamoImplementation::CheckParticleTypes(KIM_API_model* const pkim,
                                                 int const* const particleTypes)
    const
{
  int ier;
  for (int i = 0; i < cachedNumberOfParticles_; ++i)
  {
    if ((particleTypes[i] < 0) || (particleTypes[i] >= numberModelTypes_))
    {
      ier = KIM_STATUS_FAIL;
      pkim->report_error(__LINE__, __FILE__,
                         "unsupported particle types detected", ier);
      return ier;
    }
  }
