...
def plotSubSet(indices, energies,index0,poleaxis,params,structure='fcc',correction=0):
    ...
    for i in range(0,len(indices)):
        ...
        energy = energies[i]
        if numpy.abs(numpy.dot(index,index0)) < 1e-13:
            cos = numpy.dot(poleaxis,[h,k,l])/(numpy.sqrt((h**2+k**2+l**2)*numpy.dot(poleaxis,poleaxis)))
            sin = numpy.linalg.norm(numpy.cross(poleaxis,[h,k,l]))/(numpy.sqrt((h**2+k**2+l**2)*numpy.dot(poleaxis,poleaxis)))
            angle = numpy.arctan2(sin, cos)*180./numpy.pi

            #print numpy.dot(index0, numpy.cross(poleaxis, n_vector) / numpy.sqrt(numpy.dot(poleaxis, poleaxis)))
    ...
    for i in xrange(len(angles)-1):
        n0 = numpy.array(angles_dict[angles[i]])
        n1 = numpy.array(angles_dict[angles[i+1]])

        ns = [n0 + (n1-n0)*j for j in numpy.linspace(0.0, 1.0, 500)]
        cos = [numpy.dot(poleaxis,nvec)/(numpy.sqrt(numpy.dot(nvec,nvec)*numpy.dot(poleaxis,poleaxis))) for nvec in ns]
        sin = [numpy.linalg.norm(numpy.cross(poleaxis,nvec))/(numpy.sqrt(numpy.dot(nvec,nvec)*numpy.dot(poleaxis,poleaxis))) for nvec in ns]
        nangle = [180./numpy.pi * numpy.arctan2(s, c) for s,c in zip(sin, cos)]

        fit_angles.extend([tn for tn in nangle])
        fit_energies.extend([ fitFunction(list(nvec), params, correction, structure) for j,nvec in enumerate(ns)])
